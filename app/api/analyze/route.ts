...
async function getKeywordMetricsLive(keyword: string, login: string, password: string) {
  ...
  if (!response.ok) {
    console.warn('[DataForSEO] Metrics failed, using defaults');
  ...
  console.log(`[DataForSEO] Metrics: vol=${metrics.search_volume}, cpc=${metrics.cpc}, comp=${metrics.competition}`);
